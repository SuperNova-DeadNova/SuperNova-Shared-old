...
sndir="./"							##  Folder containing SuperNova.exe
...
gamename="RedCraft - SuperNova"		##  Arbitrary name of server, will not affect actual server name
gamepid="${sndir}/sn.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${sndir}/sn.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${sndir}/SuperNova.exe" ]; then
	echo -e "cannot find ${sndir}/SuperNova.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${sndir}/SuperNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${sndir}/SuperNova.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${sndir}/SuperNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${sndir}/SuperNova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${sndir}/SuperNova.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${sndir}/SuperNova_.update" ]; then
			if [ -f "${sndir}/SuperNova.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${sndir}/SuperNova.exe
				rm ${sndir}/SuperNova_.dll
				mv ${sndir}/SuperNova.update ${sndir}/SuperNova.exe
				mv ${sndir}/SuperNova_.update ${sndir}/SuperNova_.dll
				if [ -f "${sndir}/SuperNova_.update" ]; then
					if [ -f "${sndir}/SuperNova.update" ]; then
						echo -e FAILED!
						if [ -f "${sndir}/SuperNova_.dll" ]; then
							if [ -f "${sndir}/SuperNova.exe" ]; then
								$0 ${default}
							fi
						else
